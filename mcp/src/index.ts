...
/**
 * 排版慣例提示，接在「筆記內容」類參數說明之後，讓 AI 透過 MCP 寫筆記時就依全站慣例排版
 * （對應後端 NoteFormatConventions；跨語言無法共用常數，故在此維護對應文字，兩邊要一起改）。
 */
const TOGGLE_HINT =
  '。排版慣例：'
  + '①【每個標題都摺疊】把每一個標題（#／##／###）都改寫成各自的摺疊區塊——用一行 :::toggle 標題文字（不含 # 符號）起頭、放入該標題底下的內容、再用單獨一行 ::: 收尾；'
  + '標題有階層就巢狀（上層標題的摺疊裡直接放下層標題的摺疊，收尾的 ::: 由內而外各一行、一律用三個冒號），預設收合、想預設展開用 :::toggle-open。'
  + '②【句子換行】每句話結束（。？！之後）就換行，且行尾補「兩個半形空格」再換行，避免單行橫向過長（表格儲存格內、程式碼區塊內不套用）。'
  + '③長證據/完整程式碼或指令/延伸補充可再摺疊收納，但重點與結論要留在外面、摺疊標題要能一眼看出內容'
