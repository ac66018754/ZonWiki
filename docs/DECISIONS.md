...
- **最終決定**：③。工具鍵照工具列排列順序用數字 1-9（Excalidraw 同款心智模型）：1 畫筆…9 框選橡皮擦；英文字首字母 c 目錄（僅筆記）、s 便利貼、i 圖片板。刻意**不配鍵**：結束繪圖（滑鼠右鍵已有）、清除全部（破壞性防誤觸）、儲存快照（低頻）。
- **關鍵取捨與防呆**：
...
- **驗證**：單元 30 例（鍵位規格、跨/同 scope 衝突、Runtime 派發矩陣含 repeat 過濾、對應完備性、鍵帽 hook 改鍵更新）；Playwright 對 production build 實測 20 例全過（含收合態啟用、編輯頁/編輯彈窗不觸發、畫布工具箱關閉按 6 不展開、改鍵後提示即時更新、repeat 連發不觸發、Row1 單行）；鍵帽提示對比實測 warmpaper 5.21:1、dark 5.62:1（≥4.5 AA）。
