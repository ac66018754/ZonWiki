...
- **⏱ 時間追蹤面板（可收合）**：記錄每天把時間花在什麼上面——輸入項目名稱＋可選分類（autocomplete）→「▶ 開始」計時；進行中項目顯示**即時經過時間**、一鍵「⏹ 結束」自動算時間差；**開始/結束時間皆可事後編輯**（不一定要即時操作），並可為每筆加**備註**（📝，最長 1000 字）。歷史檢視依 **日/週/月/年** 分段切換＋前後期導覽（週/月依日分組、年依月分組，各附小計、期間總計與**分類小計**）。整塊可收合（記 localStorage）；刪除＝軟刪除、垃圾桶可還原；編輯會進活動紀錄。時間一律存 UTC、依使用者時區顯示與歸日。
  - **iPhone 主畫面免開網頁操作**（見 [docs/iOS捷徑-時間追蹤.md](./docs/iOS捷徑-時間追蹤.md)）：**iOS 捷徑**（觸發動作）＋API 權杖——開始（新增或「選既有」＋可選備註）、結束（挑進行中清單，或 `stop-latest` 一鍵結束最近的）；**Scriptable 小工具**（[docs/ios-widgets/](./docs/ios-widgets/)，被動顯示）——桌面看今日／本週統計、進行中可點列結束（二次確認防手誤）。相關端點：`GET /api/time-entries/summary?scope=day|week`（依時區歸日/週的總計/分類/進行中）、`/recent-items`（既有項目）、`/running`、`/{id}`。

### 2. 筆記（Notes）
- **新增筆記表單（由上而下）**：標題 →（分類、標籤）→ 內容（Markdown 編輯器）。分類/標籤皆可就地新增。（本系統**不使用「草稿」概念**——筆記沒有草稿/發佈之分。）
- **編輯**：Markdown 編輯器；支援 Wiki link `[[筆記標題]]` 反向連結
- **編輯模式 toggle 摺疊（2026-08-11）**：`:::toggle` 區塊在**編輯 textarea 內**也能收合，不必切預覽——編輯區左側每個 toggle 標頭有 ▾ 摺疊鈕（工具列另有「▸▸ 全部摺疊／▾▾ 全部展開」）；摺疊後該區塊收成單行徽章（如 `⋯〔已摺疊 5 行 #gk7q〕`），**點徽章（或在徽章內打字／緊鄰按 Backspace）即自動展開**。摺疊純屬編輯畫面的視圖狀態：**儲存與 onChange 永遠是展開後的完整內容**（徽章絕不入 DB、隱藏內文絕不遺失；刪除含徽章的整行＝刪除整塊，Ctrl+Z 貼回徽章可完整復原）。「重排選取（局部排版）」在選取跨到徽章時會擋下並提示先展開。設計取捨詳見 docs/DECISIONS.md 2026-08-11。
- **換行體系（2026-08-13 重整）**：
  - **Enter 即換行（全域）**：單一換行＝硬換行（Notion／GitHub 留言區式），不再需要行尾兩空白（舊寫法與行尾 `\`、`<br>` 仍相容）。後端 Markdig `UseSoftlineBreakAsHardlineBreak`＋前端 `remark-breaks` 三渲染點（筆記/便利貼/畫布節點）對齊；`RenderVersion` 3 起舊筆記自動重渲染收斂。
  - **表格儲存格換行（編輯視圖層）**：儲存永遠是單行 `<br>`（合法 GFM、跨平台互通），但**編輯 textarea 內顯示成多行**——`<br>` 展開為「換行＋對齊墊片＋`↳ ` 續行標記」；**Shift+Enter** 在儲存格內直接插入換行（背後寫 `<br>`，使用者看不到標籤；Ctrl+Z 可復原）；複製跨續行的選取會自動轉回 `<br>` 單行形（貼到 GitHub 等外部平台表格不會壞）。讀模式雙右鍵直編儲存格時也以真實換行顯示（不再看到字面 `<br>`）。設計與安全機制詳見 docs/DECISIONS.md 2026-08-13。
  - **儲存格內多 checkbox**：儲存格內容寫 `[ ] 甲<br>[x] 乙`（每段一個），讀模式渲染成**可逐項點擊勾選**的核取方塊（像 OneNote 一格多待辦）；點擊直接寫回原文。
  - **編輯頁表格 `|` 上色**：編輯 textarea 內真表格行的管線字元帶主題色色塊背景，一眼看清欄界。
...
- **手機閱讀優化（2026-08-11；定位＝電腦編輯、手機閱讀）**：閱讀頁在手機（≤768px）不再左右橫移（`body`/頁面容器 `overflow-x: clip` 兜底＋置頂工具列可換行＋長網址斷行）；**表格改「容器內橫捲」**——欄寬取自然寬、過寬時在表格自己的捲動容器內左右滑，不再被壓成一行一字；桌機拖的欄寬記憶在窄螢幕不套用、觸控裝置不掛拖寬把手（避免劫持捲動）、觸控長按選字不再被右鍵抑制吃掉；浮動繪圖工具列手機預設收合（點右下 ▴ 展開）；內文內距縮小、`100dvh` 修 iOS 底部裁切。詳見 docs/DECISIONS.md 2026-08-11。

### 3. 日程規劃 / 任務（Tasks）
- **卡片式待辦**：todo / doing / done / custom 等狀態
- **任務編輯彈窗（單一、永遠可編輯）**：開啟任務即直接是可編輯表單（不再有「檢視 →（點一下）編輯」兩段式，合併成一個彈窗）。版面：**標題置頂**；其下分兩欄——**左欄**放所有屬性（狀態／優先度／分類／標籤／開始日期／截止日期／父任務／子任務／關聯／**建立日期（唯讀**，UTC 存、依使用者時區顯示、含年份**）**，**狀態與優先度為小巧的分段鈕**），**右欄**放內容（Markdown 編輯器，工具列備齊 **H1~H3／粗・斜・刪除線／項目・編號・待辦清單／引用／行內＋區塊程式碼／表格／圖片／分隔線／連結**）。**交易式編輯**：所有欄位與子任務變更（含解除父子關係）都**只暫存於彈窗，唯有按「儲存」才寫入後端**；關閉／返回／進入子任務若有未存變更會先詢問是否放棄，放棄則全部還原（不寫入）。**進入子任務後再關閉會回到父任務**（導覽堆疊；子任務時左上顯示「←」返回、唯有根任務的「✕／Esc」才完全關閉）。子任務列每列有「↗ 開啟」「🔗 關聯」「✕ 移除父子關係」（皆暫存、按「儲存」才生效）。**✕ 只解除父子關係、不刪除任務**（會先跳確認；該任務變成獨立頂層任務，仍存在、可再設父任務）；要徹底刪除請「↗ 開啟」該任務後用編輯器的「刪除」（兩段式確認）。
- **多視圖**：
...
- **左側欄雙分頁：置頂的任務（預設）／快捷鍵介紹**：Todo 頁側欄最上方有兩個平行分頁的切換鈕——**預設顯示「📍 置頂的任務」**，點切換鈕才換到「⌨️ 快捷鍵介紹」（原本的快捷鍵清單）。
  - **任務可「置頂（Todo 側欄）」**：任務編輯器與快速新增表單的「📌 釘選到首頁」旁多了「📍 置頂（Todo 側欄）」勾選（`TaskCard_IsPinnedToTodo`，與首頁釘選**完全獨立**、無排序欄位）；勾選後該任務出現在側欄「置頂的任務」分頁（依建立時間排序）。點側欄項目即開啟該任務的完整編輯器；已完成者以刪除線顯示。任務儲存／建立／刪除後側欄即時刷新（`zonwiki:tasks-changed` 事件）。後端 `GET /api/tasks?view=list&pinnedToTodo=true`（僅 list 視圖吃此參數）。
- **鍵盤快捷鍵（可自訂改鍵）**：左側欄「快捷鍵介紹」分頁列出目前生效的鍵位。預設——**全域**（任何頁面）：`T` 前往 Todo、`Q` 開問啦、`N` 筆記、`F` 聚焦搜尋框；**Todo 頁**：`Y/M/W/D` 切年/月/週/日行事曆、`V` 循環顯示模式（清單→看板→行事曆）、`A` 彈出新增任務表單。每個動作都可在「個人頁 → 快捷鍵」重新綁定並儲存（見 §7.1）。輸入文字時不會觸發。同一鍵可在**不同頁面範圍**各綁一個動作（如 `A` 在 Todo 頁＝新增任務、筆記頁＝新增筆記），執行器依當前頁面解析。
...
| [iOS捷徑-時間追蹤.md](./docs/iOS捷徑-時間追蹤.md) | iPhone 主畫面一鍵「開始/結束」計時的捷徑設定（PAT ＋ 時間追蹤 API） |
