...
| **MCP** | 自家 MCP Server（規劃中） | 供任何 AI 助理讀寫知識庫/畫布；Phase 11 |
...
*規劃於 Phase 11*。目標是讓任何 AI 助理（Claude、ChatGPT 等）透過 MCP 介面讀寫 ZonWiki 的知識庫與畫布。
...
├─ search_notes(query)
├─ list_canvas()
├─ create_node(canvasId, data)
├─ update_node(nodeId, data)
└─ create_edge(sourceId, targetId)
```

### 啟用 MCP（Phase 11）

```powershell
# 啟動 MCP Server（獨立進程或內嵌）
dotnet run --project src/ZonWiki.Mcp

# 在 Claude Code 設定檔裡註冊
# ~/.claude/mcp-servers.json (or similar)
{
  "servers": {
    "zonwiki": {
      "command": "dotnet",
      "args": ["run", "--project", "src/ZonWiki.Mcp"],
      "env": {
        "DATABASE_URL": "postgresql://user:pass@localhost:5533/zonwiki"
      }
    }
  }
}
...
之後在 Claude 或其他 AI 助理內就能讀寫 ZonWiki 資料。
...
| 文件 | 內容 |
|---|---|
| [升級執行計畫.md](./docs/升級執行計畫.md) | **必讀**：鎖定決策、目標架構、資料模型、分階段路線圖 |
| [升級藍圖-個人知識與任務作業系統.md](./docs/升級藍圖-個人知識與任務作業系統.md) | 願景與系統定位 |
| [待討論清單.md](./docs/待討論清單.md) | 尚未決定的細節點 |
...
**Q: 如何監控正式環境？**  
A: `/healthz` + `/api/health/ready` 端點；可搭配 Prometheus / 其他監控工具；待實作。
