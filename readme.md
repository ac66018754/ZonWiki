...
- **⏱ 時間追蹤面板（可收合）**：記錄每天把時間花在什麼上面——輸入項目名稱＋可選分類（autocomplete）→「▶ 開始」計時；進行中項目顯示**即時經過時間**、一鍵「⏹ 結束」自動算時間差；**開始/結束時間皆可事後編輯**（不一定要即時操作）。歷史檢視依 **日/週/月/年** 分段切換＋前後期導覽（週/月依日分組、年依月分組，各附小計、期間總計與**分類小計**）。整塊可收合（記 localStorage）；刪除＝軟刪除、垃圾桶可還原；編輯會進活動紀錄。**iPhone 主畫面免開網頁即可「開始/結束」**：iOS 捷徑＋API 權杖呼叫 `POST /api/time-entries`、`POST /api/time-entries/stop-latest`（一鍵結束最近開始的項目），設定教學見 [docs/iOS捷徑-時間追蹤.md](./docs/iOS捷徑-時間追蹤.md)。時間一律存 UTC、依使用者時區顯示與歸日。
